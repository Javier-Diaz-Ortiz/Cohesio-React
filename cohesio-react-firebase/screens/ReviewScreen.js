...
if (photo) {
  // Posición para el texto "Comment:"
  const textX = 10;
  const textY = 160 + redRooms.length * 20; // Posición Y ajustada para el comentario
  ...
  const commentTextY = textY + 15; // Aseguramos más espacio entre el título y el comentario

  // Escribir el comentario o mensaje predeterminado
  doc.setFontSize(12);
  doc.setFont("helvetica", "normal");
  ...
  doc.text(
    ...
    15,
    commentTextY,
    { maxWidth: 180 } // Ajustar texto largo al ancho
  );

  // Ajustar la posición para la imagen, debajo del comentario
  ...
  // Verificar si la imagen excede la altura del contenedor y ajustarla
  if (imageHeight > containerHeight) {
    imageHeight = containerHeight * scaleFactor; // Ajustar la altura si es necesario
    imageWidth = imageHeight * imageAspectRatio; // Mantener la relación de aspecto
  }

  // Calcular coordenadas para centrar la imagen dentro del área disponible
  const imageX = textX + (containerWidth - imageWidth) / 2; // Centrado horizontal
  const imageY = imageYPosition; // Colocar la imagen debajo del comentario

  // Validar las coordenadas antes de agregar la imagen
  if (
    !isNaN(imageX) &&
    !isNaN(imageY) &&
    !isNaN(imageWidth) &&
    ...
    // Añadir la imagen al PDF con el tamaño reducido
